...
export class LsfgService {
	...
	static detectLosslessDll(): string {
		...
		const candidates = [
			...
			path.join(home, '.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Lossless Scaling/Lossless.dll')
		];
	...
	static async getLsfgProfileForGame(name: string, gamePath: string): Promise<LsfgProfileData | null> {
...
`;
	...
	static async installLsfg(onProgress: (percent: number, message: string) => void): Promise<void> {
		...
		const releaseData: any = await new Promise((resolve, reject) => {
			https.get(releasesUrl, { headers: { 'User-Agent': 'LightLauncher-App' } }, res => {
		...
		await new Promise<void>((resolve, reject) => {
			https.get(downloadURL, { headers: { 'User-Agent': 'LightLauncher-App' } }, res => {
				if (res.statusCode === 302 || res.statusCode === 301) {
					if (res.headers.location) {
						return https.get(res.headers.location, { headers: { 'User-Agent': 'LightLauncher-App' } }, res2 => {
