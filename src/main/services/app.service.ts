...
export class AppService {
	...
	static async runSystemPicker(title: string, isFolder: boolean, filters?: { displayName: string; pattern: string }[]): Promise<string | null> {
		// Zenity
		try {
		...
		try {
			...
			if (isFolder) {
				...
				let filterStr = '';
				if (filters && filters.length > 0) {
					filterStr = filters.map(f => `${f.displayName} (${f.pattern.replace(/;/g, ' ')})`).join(';;');
				}
				const { stdout } = await execAsync(`kdialog --getopenfilename . "${filterStr}" --title "${title}"`);
				return stdout.trim() || null;
			}
		} catch {}
	...
	static async pickFileCustom(title: string, filters: { displayName: string; pattern: string }[]): Promise<string> {
